...
package struct Chord: Equatable {
    package enum Quality: Equatable {
        case major
        case minor
    }
    package enum Extension: Equatable {
        case none
        case dom7      // +10 from root
        case maj7      // +11 from root
    }

    package let rootPitchClass: Int   // 0 (C) .. 11 (B)
    package let quality: Quality
    package let ext: Extension

    // Semitone intervals (from root) that make up this chord.
    package var intervals: [Int] {
    ...
    package static let noteNames = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]
...
package enum ChordParseError: Error, CustomStringConvertible {
    case emptyInput
...
package enum ChordParser {
    package static func parse(_ raw: String) throws -> Chord {
