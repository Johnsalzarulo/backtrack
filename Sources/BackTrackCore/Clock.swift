...
package final class Clock: ObservableObject {
    package let state: AppState
    package let audio: AudioEngineController
    ...
    package init(state: AppState, audio: AudioEngineController) {
        self.state = state
        self.audio = audio
    }
    ...
    // for songs.
    package func startMotif(_ motif: CountdownMotif) {
        stopMotif()
        activeMotif = motif
    ...
    package func nextPart() { stepPart(by: 1) }
    package func previousPart() { stepPart(by: -1) }
    ...
    private func stepPart(by direction: Int) {
        ...
        if state.isPlaying {
            ...
            state.currentPartIndex = PartNavigation.wrappedIndex(
                current: state.currentPartIndex,
                direction: direction,
                count: count
            )
            state.currentBar = 0
    ...
    private func stopAsync(songFinished: Bool = false) {
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.stop()
            if songFinished && self.state.platformCapabilities == .performOnly {
                self.state.advanceLineupCursor?()
            }
        }
    }
